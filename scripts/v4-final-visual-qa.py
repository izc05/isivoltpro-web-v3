...
ROUTES = [
    ("hub", "preview-v4/"),
    ("home", "preview-v4/home/"),
    ("producto", "preview-v4/producto/"),
    ("soluciones", "preview-v4/soluciones/"),
    ("apps", "preview-v4/apps/"),
    ("sectores", "preview-v4/sectores/"),
    ("recursos", "preview-v4/recursos/"),
    ("blog", "preview-v4/blog/"),
    ("empresa", "preview-v4/empresa/"),
    ("demo", "preview-v4/demo/"),
    ("precios", "preview-v4/precios/"),
    ("contacto", "preview-v4/contacto/"),
    ("acceso", "preview-v4/acceso/"),
    ("seguridad", "preview-v4/seguridad/"),
    ("privacidad", "preview-v4/privacidad/"),
    ("social", "preview-v4/social/"),
    ("fase-10", "preview-v4/fase-10/"),
]
VIEWPORTS = [
    ("mobile-390", 390, 844),
            ...
            for name, route in ROUTES:
                ...
                try:
                    ...
                    if widest > metrics["clientWidth"] + 2:
                        failures.append(
                            f"{viewport_name} {name}: overflow horizontal {widest}px > {metrics['clientWidth']}px"
                        )

                    broken_images = page.locator("img").evaluate_all(
                        "els => els.filter(img => !img.complete || img.naturalWidth < 1).map(img => img.currentSrc || img.src)"
                    )
                    if broken_images:
                        failures.append(f"{viewport_name} {name}: imágenes rotas: {broken_images[:3]}")

                    missing_alt = page.locator("img").evaluate_all(
                        "els => els.filter(img => !img.hasAttribute('alt') || !img.getAttribute('alt').trim()).map(img => img.currentSrc || img.src)"
                    )
                    if missing_alt:
                        failures.append(f"{viewport_name} {name}: imágenes sin ALT: {missing_alt[:3]}")
                    ...
                    page.screenshot(
                        path=str(OUT / f"{viewport_name}-{name}.jpg"),
                        full_page=True,
                        type="jpeg",
                        quality=72,
                    )

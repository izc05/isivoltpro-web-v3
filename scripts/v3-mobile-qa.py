...
def capture_entry(browser)->None:
    ...
    metrics=page.evaluate("""()=>{const root=document.documentElement;const visible=(el)=>{if(!el)return false;const s=getComputedStyle(el),r=el.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>0&&r.height>0};const intro=document.querySelector('.entry3d'),canvas=document.querySelector('#entry3d-canvas'),scene=document.querySelector('#entry3d-scene'),logo=document.querySelector('.entry3d__logo'),skip=document.querySelector('#skip-intro');const sr=scene?.getBoundingClientRect(),lr=logo?.getBoundingClientRect(),kr=skip?.getBoundingClientRect(),style=intro?getComputedStyle(intro):null;return{introVisible:visible(intro)&&!intro.classList.contains('is-done'),canvasW:canvas?.width||0,canvasH:canvas?.height||0,sceneW:sr?.width||0,sceneH:sr?.height||0,logoVisible:visible(logo),logoW:lr?.width||0,logoH:lr?.height||0,skipVisible:visible(skip),skipH:kr?.height||0,legacyCount:document.querySelectorAll('.entry3d__heading,.entry3d__note,.entry3d__actions,.entry3d__phone,.entry3d__orders').length,bgColor:style?.backgroundColor||'',scrollWidth:root.scrollWidth,clientWidth:root.clientWidth}}""")
